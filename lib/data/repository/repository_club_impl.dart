...
class RepositoryClubImpl extends RepositoryClub{
  ...
  Future<Club> getClub(int clubId) {
    return ApiClient.getRequest(endPoint: "/v3/clubs/$clubId", dataConstructor: (data)=>Club.fromJson(Map<String,dynamic>.from(data)));
  }

  @override
  Future<List<SummaryClub>> getLoggedInAthleteClubs(int page, int perPage) {
    return ApiClient.getRequest(endPoint: "v3/athlete/clubs", dataConstructor: (data){
      if(data is List){
        return data.map((e) => SummaryClub.fromJson(Map<String,dynamic>.from(e))).toList();
      }
      return [];
    });
  }
  ...
  Future<List<SummaryActivity>> listClubActivities(int clubId, int page, int perPage) {
    return ApiClient.getRequest(endPoint: "v3/clubs/$clubId/activities", dataConstructor: (data){
      if(data is List){
  ...
  Future<List<SummaryAthlete>> listClubAdministrators(int clubId, int page, int perPage) {
    return ApiClient.getRequest(endPoint: "v3/clubs/$clubId/admins", dataConstructor: (data){
      if(data is List){
  ...
  Future<List<SummaryAthlete>> listClubMembers(int clubId, int page, int perPage) {
    return ApiClient.getRequest(endPoint: "v3/clubs/$clubId/members", dataConstructor: (data){
      if(data is List){
        return data.map((e) => SummaryAthlete.fromJson(Map<String,dynamic>.from(e))).toList();
      }
      return [];
